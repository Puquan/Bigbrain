...
function QuestionFormEdit () {
  ...
  const [newQuestion, setNewQuestion] = React.useState<any>(null);
  ...
  React.useEffect(() => {
    if (QuestionfromDB) {
      for (let i = 0; i < QuestionfromDB.length; i++) {
        if (QuestionfromDB[i].questionId === Number(questionId)) {
          setNewQuestion(QuestionfromDB[i]);
          setIndex(i);
        }
      }
    }
